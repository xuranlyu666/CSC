...
public class Heap <E> extends AbstractQueue<E> {
  ...
  // heap is complete - need to have left child but no need to have right child
  ...
  */
  public Heap (Comparator < E > comp) {
    comparator = comp; //update the previously declared null comparator with the input comp
  }

  public int size () { return theElements.size(); } // a method of array list
  ...
  */
  public E poll() {
    ...
    if(theElements.size() == 1){
      return theElements.remove(theElements.size()-1);
    ...
    ///
    theElements.set(0,theElements.remove(theElements.size()-1));
    ///
    ...
    // If the right child is there and is lesser than the left child...
        ...
        if (isValid(right(index)) && compare(right(index), left(index)) < 0){
          ...
          index = right(index);// Swap with right child (and set index).
  ...
  public boolean remove (Object o) {
    ...
    ///
    theElements.set(index, theElements.remove(size()-1)); //set the index with the last removed?
    ...
    ///
    while (2 * index + 1 < size() && compare(index, left(index)) > 0 || (2 * index +2 < size() && compare(index, right(index)) > 0)){
      if ( 2 * index + 1 < size() && compare(right(index), right(index))<0){
        swap(index, right(index));
        index = right(index);
        continue;
      }
